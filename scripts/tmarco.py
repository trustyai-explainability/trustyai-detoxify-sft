...
class TMaRCo:
    ...
    def __init__(self, base_model=None, expert_weights=None, tokenizer=None, max_length=150,
                 model_type: str = 'causal_lm', device=None):
    ...
    def rephrase(self, originals: list, scores: list = None, masked_outputs: list = None, compute_probs: bool = False,
        ...
        for in_idx in range(len(originals)):
            ...
            if len(original.strip()) == 0:
                ...
                if combine_original:
                    original_sentence_tokens = self.tokenizer(original)["input_ids"].to(self.device)[1:-1]
                fmp_experts = []
                if compute_probs:
                    for expert in self.experts:
                        fmp_experts.append(pipeline(
                            "fill-mask", 
                            model=expert, 
    ...
    def compute_mask_logits_multiple(self, model, sequences, verbose: bool = False, mask: bool = True):
        ...
        subseq_ids = self.tokenizer(sequences, return_tensors="pt", padding=True).to(self.device)
        if verbose:
            raw_outputs = model.generate(**subseq_ids)
            print(sequences)
            ...
            if len(masked_sentences) == 0:
                ...
                for model in self.experts:
                    ...
                    if use_logits:
                        ...
                        fmp = pipeline("fill-mask", model=model, tokenizer=self.tokenizer, top_k=10, device=self.device)
                ...
                for distr_pair in distr_pairs:
                    js_distance = jensenshannon(distr_pair[0].cpu().clone().numpy(), distr_pair[1].cpu().clone().numpy(), axis=1)
                    if normalize:
    ...
    def reflect(self, texts: list, threshold=1.2, verbose=False, messages=None, user_prompt=default_user_prompt,
        ...
        converse_pipeline = pipeline("conversational", model=chat_model, tokenizer=chat_tokenizer, device=self.device)
